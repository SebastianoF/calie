...
def test_vf_identity_eulerian_wrong_input():
    with assert_raises(IOError):
        vf_identity_eulerian((3, 3, 3, 3, 3))
...
def test_vf_identity_eulerian_ok_2d():
    ...
    assert_array_equal(vf_identity_eulerian((10, 9)), expected_vf)
...
def test_vf_identity_eulerian_ok_3d():
    ...
    assert_array_equal(vf_identity_eulerian((10, 9, 8)), expected_vf)
...
def test_vf_identity_lagrangian_like_wrong_input():
    with assert_raises(IOError):
        vf_identity_lagrangian_like(np.ones([4, 4, 4, 4]))


def test_vf_identity_lagrangian_like_ok_2d():
    assert_array_equal(vf_identity_lagrangian_like(np.ones((10, 9, 1, 1, 2))), np.zeros((10, 9, 1, 1, 2)))


def test_vf_identity_lagrangian_like_ok_3d():
    assert_array_equal(vf_identity_lagrangian_like(np.ones((10, 9, 8, 1, 3))), np.zeros((10, 9, 8, 1, 3)))
...
def test_vf_identity_eulerian_like_wrong_input():
    with assert_raises(IOError):
        vf_identity_eulerian_like(np.zeros([3, 3, 3, 3, 7]))
...
def test_vf_identity_eulerian_like_ok_2d():
    ...
    assert_array_equal(vf_identity_eulerian_like(np.ones((10, 9, 1, 1, 2))), expected_vf)
...
def test_vf_identity_eulerian_like_ok_3d():
    ...
    assert_array_equal(vf_identity_eulerian_like(np.ones((10, 9, 8, 1, 3))), expected_vf)
...
def test_vf_identity_matrices_test_shape():
    ...
    vf_id = vf_identity_matrices(omega, t=4)
    assert_array_equal(vf_id.shape, expected_shape)

    omega = (5, 6, 7)
    expected_shape = (5, 6, 7, 12, 9)
    vf_id = vf_identity_matrices(omega, t=12)
    assert_array_equal(vf_id.shape, expected_shape)


def test_vf_identity_matrices_test_values():
    omega = (5, 4, 3)
    vf_id = vf_identity_matrices(omega, t=2)
...
def test_from_image_to_omega_by_non_existing_path():
    with assert_raises(IOError):
        from_image_to_omega('z_spam_folder')
...
def test_from_image_to_omega_by_path():
    ...
    obtained_omega = from_image_to_omega(pfi_im)
...
def test_from_image_to_omega_by_nifti():
    ...
    obtained_omega = from_image_to_omega(nib_im)
    assert_array_equal(obtained_omega, expected_omega)
...
def test_vf_identity_lagrangian_like_image():
    ...
    id_lagrangian = vf_identity_lagrangian_like_image(nib_im)
...
def test_vf_identity_eulerian_like_image():
    omega = (30, 29, 28)
    nib_im = nib.Nifti1Image(np.ones(omega), affine=np.eye(4))
    id_eulerian = vf_identity_eulerian_like_image(nib_im)
