...
"""
Module for the computation of a dataset of 2d linear generated with linear matrices.
...
if __name__ == '__main__':
    ...
    params.update({'tastes'          : [1, 2, 3, 4, 5]})
    params.update({'beta'            : 0.1})
    ...
    if control['generate_dataset']:
        ...
        print('--------------------------------------------------------------------------')
        print('Generating dataset GL2! filename: linear-<s>-<algebra/group>.npy j = 1,...,N ')
        print('--------------------------------------------------------------------------')

        for s in range(params['num_samples']):  # sample s

            # Generate SVF
            dm1 = params['beta'] * linear.randomgen_linear_by_taste(1, np.random.choice(params['tastes']), omega)
            ...
            pfi_svf0 = jph(pfo_output_A4_GL2, 'gl2-{}-algebra.npy'.format(s + 1))
            pfi_flow = jph(pfo_output_A4_GL2, 'gl2-{}-group.npy'.format(s + 1))
    ...
    print('--------------------------------------------------------------------------')
    print('Compute exponentials GL2! filename: gl2-<method>-STEPS_<steps>.csv        ')
    print('--------------------------------------------------------------------------')
    ...
    if control['get_statistics']:
        ...
        # in a file called gl2-stats-<method>.csv
